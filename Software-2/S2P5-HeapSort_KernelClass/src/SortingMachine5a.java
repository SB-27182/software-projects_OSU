...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (leftExists) {
            ...
            if (leftExists && rightExists) {
                if (order.compare(array.entry(leftIndex),
                        array.entry(rightIndex)) <= 0) {
                    smallerIndex = leftIndex;
                } else {
            ...
            if (order.compare(array.entry(top),
                    array.entry(smallerIndex)) > 0) {

                array.exchangeEntries(top, smallerIndex);
    ...
    public final int size() {
        ...
        assert this.conventionHolds();
        if (this.insertionMode) {
            retVal = this.entries.length();
        } else {
            retVal = this.heapSize;
        }
        return retVal;
