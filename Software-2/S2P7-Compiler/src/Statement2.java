...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Sequence<Tree<StatementLabel>> thisChildren = this.rep
                .newSequenceOfTree();
        StatementLabel thisStatementLabel = this.rep.disassemble(thisChildren);

        Statement2 localS = (Statement2) s;
        thisChildren.add(pos, localS.rep);

        this.rep.assemble(thisStatementLabel, thisChildren);
    ...
    public final Statement removeFromBlock(int pos) {
         ...
         * code.
         */
        Statement2 s = this.newInstance(); //has statementLabel

        Sequence<Tree<StatementLabel>> thisChildren = this.rep
                .newSequenceOfTree();
        StatementLabel thisStatementLabel = this.rep.disassemble(thisChildren);
        Tree<StatementLabel> tree = thisChildren.remove(pos);
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        StatementLabel labelRoot = new StatementLabel(Kind.IF_ELSE, c);

        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        Statement2 s1Local = (Statement2) s1;
        Statement2 s2Local = (Statement2) s2;
        Tree<StatementLabel> s1Tree = s1Local.rep;
        Tree<StatementLabel> s2Tree = s2Local.rep;

        children.add(0, s1Tree);
        children.add(1, s2Tree);

        this.rep.assemble(labelRoot, children);
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();
        StatementLabel labelRoot = this.rep.disassemble(children);

        s1Local.rep = children.remove(0);
        s2Local.rep = children.remove(0);

        this.createNewRep();
        return labelRoot.condition;
    }
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        StatementLabel labelRoot = new StatementLabel(Kind.WHILE, c);

        Sequence<Tree<StatementLabel>> children = this.rep.newSequenceOfTree();

        Statement2 s1Local = (Statement2) s;
        Tree<StatementLabel> s1Tree = s1Local.rep;
    ...
    public final void assembleCall(String inst) {
        ...
        StatementLabel labelRoot = new StatementLabel(Kind.CALL, inst);
