...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String overlapS1B = str1.substring(str1.length() - overlap, str1.length());
        String overlapS2F = str2.substring(0, overlap);
        if (overlapS1B.contains(overlapS2F)) {
            truncated = str2.substring(overlap);
            returnS = str1 + truncated;
        } else {
            truncated = str1.substring(overlap);
            returnS = str2 + truncated;
        }
        /*
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        Set<String> sTemp = new Set1L<String>();
        Set<String> sSub = new Set1L<String>();
        sTemp.transferFrom(strSet);
        while (sTemp.size() > 0) {
            String word = sTemp.removeAny();
            for (String word1 : sTemp) {
                assert !word.contains(word1)
                : "Violation of: CONTAINS_NO_SUBSTRING_PAIRS(strSet)";
                }
            for (String word1 : strSet) {
                assert !word.contains(word1)
                : "Violation of: CONTAINS_NO_SUBSTRING_PAIRS(strSet)";
            }
            strSet.add(word);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> sTemp = new Set1L<String>();
        Set<String> sSub = new Set1L<String>();
        Set<String> sSub1 = new Set1L<String>();
        String test = "";
        while (!input.atEOS()) {
            test = input.nextLine();
            if (test.length() > 1) {
        ...
        sTemp.transferFrom(s);
        while (sTemp.size() > 0) {
            String word = sTemp.removeAny();
            for (String word1 : sTemp) {
                if (!sSub.contains(word1)) {
