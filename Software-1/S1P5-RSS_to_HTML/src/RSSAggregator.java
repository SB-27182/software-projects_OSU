...
public final class RSSAggregator {
   ...
   out.print(channel.child(getChildElement(channel, "description")).child(0).label());
    ...
    /**
     * Processes one RSS 2.0 file and outputs an HTML file.
     *
     * @param file
     *           Output steam
     * @param url
     *            XML 2.0 URL link
     */
    private static void processOneXml(String file, String url) {
        boolean c = false;
        ...
        if (!c) {
            outFile.print("Not RSS version 2.0");
    ...
    public static void main(String[] args) {
        ...
        out.println("Enter valid XML with top level tag: \"feeds\"");
        xml = in.nextLine();
        XMLTree feeds = new XMLTree1(xml);

        if ((feeds.label().equals("feeds"))
        && (feeds.hasAttribute("title"))) {
            c = true;
        }

        while (!c) {
        out.print("Requires XML file with <feeds> and  attribute \"title\" ");
        xml = in.nextLine();
        feeds = new XMLTree1(xml);
        ...
        out.println("Enter output HTML index file name");
        index = in.nextLine();
        SimpleWriter outFile2 = new SimpleWriter1L(index);

        outFile2.print("\r\n<html>\r\n<head>\r\n<title>Top Stories</title>"
                        + "\r\n</head>\r\n<body>\r\n<h2>Top Stories</h2>");
        outFile2.print("\r\n<ul>");

        int feed0 = getChildElement(feeds, "feed");
        int i = feed0;
